...
local function cycle_previous_buffer()
  local buffers = get_valid_buffers()
  local current = vim.fn.bufnr '%'
  local index = vim.fn.index(buffers, current)
  if index > 0 then
    vim.cmd('buffer ' .. buffers[index == 0 and #buffers or index])
  else
    vim.cmd('buffer ' .. buffers[#buffers]) -- Wrap to last buffer
  end
end

-- Cycle to the next buffer
local function cycle_next_buffer()
  local buffers = get_valid_buffers()
  local current = vim.fn.bufnr '%'
  local index = vim.fn.index(buffers, current)
  if index < #buffers - 1 then
    vim.cmd('buffer ' .. buffers[index + 2])
  else
    vim.cmd('buffer ' .. buffers[1]) -- Wrap to first buffer
  end
end

vim.keymap.set('n', '<A-h>', cycle_previous_buffer, { desc = 'Cycle to previous buffer' })
vim.keymap.set('n', '<A-l>', cycle_next_buffer, { desc = 'Cycle to next buffer' })
